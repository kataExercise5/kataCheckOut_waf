...
namespace ScannerLib.Tests
{
    ...
    public class ScannerTests
    {
        ...
        {
            Assert.IsTrue(scanner.checkOutItem(3, tideItemName, 2) == 9.54m);
        ...
        {
            Assert.IsTrue(scanner.checkOutItem(3, applesItemName, 8.5m) == 4.25m);
        ...
        public void checkOutItemExistsWithMultipleItems()
        {
            Assert.IsTrue(scanner.checkOutItem(3, applesItemName, 8.5m) == 4.25m);
            Assert.IsTrue(scanner.checkOutItem(3, tideItemName, 2) == 9.54m);
            Assert.IsTrue(scanner.getCheckOutTotal(3) == 13.79m);
        }
        ...
        public void checkOutItemDoesNotExist()
        {
            try
            {
                decimal price = scanner.checkOutItem(3, "unknown", 2);
                Assert.Fail();
            }
            catch (Exception ex)
            {
